...
func main() {
	...
	// Create Subscriber
	sub := messaging.NewSubscriber(client)
	...
	if cfg.Middleware.Tracing.Enabled {
		shutdown, err := telemetry.InitTracer(config.TracingConfig{
			Enabled:     true,
			ServiceName: "nats-subscriber",
			Exporter:    "stdout",
		})
		if err != nil {
			log.Printf("Failed to init tracer: %v", err)
		...
		sub.Use(messaging.TracingMiddleware(otel.Tracer("nats-subscriber")))
	}
	...
	_, err = sub.Subscribe(context.Background(), topic, handler.HandleMessage, opts)
