# REST-NATS Microservice Template

This template demonstrates a **production-grade REST-NATS service** that bridges Synchronous (REST) and Asynchronous (NATS) communication.
...
### 2. Run the REST-NATS Service
```bash
go run templates/rest-nats-service/cmd/server/main.go
```
...
```bash
docker build -f templates/rest-nats-service/Dockerfile -t rest-nats-service:latest .
docker run -e NATS_URL=nats://host.docker.internal:4222 -p 8080:8080 rest-nats-service:latest
```
